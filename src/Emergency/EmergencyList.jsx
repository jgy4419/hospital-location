...
function EmergencyList(){
    ...
    // 응급실 기본정보들 (수정 중)
    let [hospitalBasicData, setHospitalBasicData] = useState([]);
        ...
        let interval = setInterval(() => {
            ...
            if(state[3].hospitalBasicData[0].기관명 !== undefined){
                for(let i = 0; i < state[3].emeCount; i++){
                    ...
                    setBasicData(hospitalBasicData);
                    ...
                    array.push(
                        <>
                            <div className="list" onClick={() => {detailInformationOpen(i)}}>
                                <div class="hospitalDetailBox">
                                    <p className="hospitalCount" style={{display: 'none'}}>{i}</p>
                                    <p className = "name">이름: {basicData[i].기관명}</p>
                                    <p className = "tel">전화번호: {basicData[i].대표전화1}</p>
                                    <p className = "address">주소: {basicData[i].주소}</p>
                                </div>
                            </div>
                        </>
                    )
